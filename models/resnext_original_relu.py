...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, num_group=32):
        ...
        # self.conv1 = conv3x3(in_planes=inplanes, out_planes=planes*2, stride=stride)
        self.conv1 = conv3x3(in_planes=inplanes, out_planes=planes, stride=stride)
        # self.bn1 = nn.BatchNorm3d(planes*2)
        self.bn1 = nn.BatchNorm3d(planes)
        self.relu = nn.ReLU(inplace=True)
        # self.conv2 = conv3x3(in_planes=planes*2, out_planes=planes*2, groups=num_group)
...
class ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, n_features,
        ...
        if (linear_units is None) or (len(linear_units) == 0):
            ...
            linear_units = [output_channel] + linear_units   # [512] + [20, 10] = [512, 20, 10]
            for i in range(len(linear_units) - 1):
                ...
                if self.clinical_variables_position + 1 == i:
                    self.linear_layers.add_module('linear%s' % i,
                                                  torch.nn.Linear(
                                                      in_features=linear_units[i] + clinical_variables_linear_units[
                                                          -1],
...
def get_resnext_original_relu(model_depth, channels, n_features, num_classes,
                              dropout_p, linear_units, clinical_variables_position, clinical_variables_linear_units,
                              clinical_variables_dropout_p, use_bias):
    assert model_depth in [10, 18, 34, 50, 101, 152, 200]

    if model_depth == 10:
        model = ResNeXt(BasicBlock, [1, 1, 1, 1], input_channels=channels, n_features=n_features, num_classes=num_classes,
        ...
        model = ResNeXt(BasicBlock, [2, 2, 2, 2], input_channels=channels, n_features=n_features, num_classes=num_classes,
                        dropout_p=dropout_p, linear_units=linear_units,
                        clinical_variables_position=clinical_variables_position,
                        clinical_variables_linear_units=clinical_variables_linear_units,
                        clinical_variables_dropout_p=clinical_variables_dropout_p,
