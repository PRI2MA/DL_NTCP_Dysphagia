...
def run_logistic_regression(features_csv, patient_id_col, baseline_col, submodels_features, features, lr_coefficients,
    ...
    # Construct features
    # NEW
    # df.loc[:, baseline_col + '_little'] = (df.loc[:, baseline_col] == 2).astype(int)
    # df.loc[:, baseline_col + '_moderate_to_severe'] = (df.loc[:, baseline_col] >= 3).astype(int)
    # df.loc[:, 'Parotid_meandose_adj'] = (df.loc[:, 'Parotid_L_meandose'] ** (1 / 2) +
    #                                      df.loc[:, 'Parotid_R_meandose'] ** (1 / 2))
    # OLD
    # df.loc[:, baseline_col] = preprocess_baseline(df.loc[:, baseline_col])
