...
def main_array():
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        for patient_id in tqdm(patients_list):
            ...
            # # Concatenate different Numpy array to (3, num_slices, num_columns, num_rows)
            # # assert ct_arr.shape == resampled_rtdose_arr.shape == segmentation_map_arr.shape
            # input_arr = np.concatenate((ct_arr, resampled_rtdose_arr, segmentation_map_arr), axis=0)
            # logger.my_print('\tinput_arr.shape (final): {}'.format(input_arr.shape))
            # logger.my_print('\tinput_arr.dtype (final): {}'.format(input_arr.dtype))
            #
            # # Save as Numpy array
            # np.save(
            #     file=os.path.join(save_dir_dataset_full, cfg.filename_patient_id_npy.format(patient_id=patient_id)),
            #     arr=input_arr)
            # Save as Numpy array
...
def main_dataset():
    ...
    # Taste
    # save_dir_dataset = '//zkh/appdata/RTDicom/Hendrike Neh/dataset_taste_new'
    # Dyspaghia:
    # save_dir_dataset = '//zkh/appdata/RTDicom/Suzanne de Vette/7. Dysphagia model/dataset_old'
    patient_id_col = cfg.patient_id_col
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        # Dyspaghia:
        # df = pd.read_csv(os.path.join(save_root_dir, 'stratified_sampling_dysphagia.csv'), sep=';')
        ...
        for label, patients_list in labels_patients_dict.items():
            ...
            for patient_id in tqdm(patients_list):
                ...
                # src = os.path.join(save_dir_dataset_full, cfg.filename_patient_id_npy.format(patient_id=patient_id))
                # dst = os.path.join(save_dir_dataset_label, cfg.filename_patient_id_npy.format(patient_id=patient_id))
                # copy_file(src, dst)
                src = os.path.join(save_dir_dataset_full, patient_id)
                # TODO: temporary
                # src = os.path.join('//zkh/appdata/RTDicom/PRI2MA/MDACC/preprocessed/dataset_full_mdacc', patient_id)
