...
def main_plot():
    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        for patient_id in tqdm(patients_list):
            ...
            # # Load npy data of shape (c, z, y, x)
            # arr = np.load(os.path.join(save_dir_dataset_full, patient_id_npy))
            # logger.my_print('\tarr.shape: {}'.format(arr.shape))
            ...
            print('ct_arr.shape:', ct_arr.shape)

            # segmentation_map for contours
            # segmentation_map_arr = arr[2]
            ...
            # TODO: TEMPORARILY ONLY CONSIDER MANDIBLE (7) / oral_cavity (9)
            # plot_arr(segmentation_map_arr == 9, nr_images=cfg.plot_nr_images, figsize=figsize,
            ...
            # TODO: TEMPORARILY ONLY CONSIDER MANDIBLE (7) / oral_cavity (9)
            # arr_list = [ct_arr, rtdose_arr, segmentation_map_arr == 9]
            # TODO: TEMPORARILY ONLY CONSIDER parotid and submandibulars
            arr_list = [ct_arr, ct_arr, rtdose_arr,
                        (segmentation_map_arr == 1) + (segmentation_map_arr == 2) +
                        (segmentation_map_arr == 3) + (segmentation_map_arr == 4)]
            ...
            segmentation_map_list = [segmentation_map_arr, segmentation_map_arr, None, None]
