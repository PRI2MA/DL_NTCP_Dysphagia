    ...
    for _, d_i in zip(mode_list, data_dir_mode_list):
        ...
        for patient_id in tqdm(patients_list):
            ...
            for structure in cfg.structures_uncompound_list:
                ...
                best_structure_value_list = [[v['Value'], v['Count']]
                                             for _, v in best_structures_value_count_dict.items() if
                                             (v['Modified_name'] == structure)]
                # If a RTSTRUCT file contains multiple v['Modified_name'] == structure, then select the structure_value
                # with highest count
                if len(best_structure_value_list) >= 2:
                    best_count = max([x[1] for x in best_structure_value_list])
                    best_structure_value_list = [x[0] for x in best_structure_value_list if
                ...
                assert len(best_structure_value_list) <= 1
                new_structure_value = cfg.structures_values[structure]
                if len(best_structure_value_list) == 1:
                    original_structure_value = best_structure_value_list[0]
