...
def get_files(sampling_type, features, filename_stratified_sampling_test_csv, filename_stratified_sampling_full_csv,
    ...
    for patient_id in patient_ids_list:
        ...
        # Features
#        print('df_features_i[features]:', df_features_i[features])
#        print('df_features_i[features].values:', df_features_i[features].values)
#        print('df_features_i[features].values.tolist():', df_features_i[features].values.tolist())
    ...
    if sampling_type == 'random':
        ...
        # # Only consider patient_ids that are available in our dataset
        # valid_patient_ids = os.listdir(config.data_dir)
        # df_split[df_split['UMCG'].astype(str).isin(valid_patient_ids)]
        ...
        for l in labels_unique:
            ...
            patient_ids_l = [x for x in patient_ids_l if int(x) not in exclude_patients]
            for p in patient_ids_l:
                df_i = df_split[df_split[patient_id_col] == p]
                print('df_i[data_preproc_config.endpoint]: ', df_i[data_preproc_config.endpoint])
                print('df_i[data_preproc_config.endpoint].values[0]): ', df_i[data_preproc_config.endpoint].values[0])
        ...
        get_df_stats(df=df_train, groups=strata_groups, mode='train', frac=train_frac, total_size=total_size, logger=logger)
        get_df_stats(df=df_val, groups=strata_groups, mode='val', frac=val_frac, total_size=total_size, logger=logger)
        get_df_stats(df=df_test, groups=strata_groups, mode='test', frac=1 - train_frac - val_frac, total_size=total_size,
                     logger=logger)
...
def get_transforms(perform_data_aug, modes_3d, modes_2d, data_aug_p, data_aug_strength, rand_cropping_size, input_size, resize_mode,
                   seg_orig_labels, seg_target_labels, device, logger):
    """
    ...
    align_corners_exception_3d = [True if mode in ['linear', 'bilinear', 'bicubic', 'trilinear'] else None for mode in modes_3d]
    ...
    generic_transforms = Compose([
        ...
        # ScaleIntensityRanged(keys=['segmentation_map'],
        ...
        # TODO: NEW
        MapLabelValued(keys=['segmentation_map'],
                       orig_labels=seg_orig_labels,
                       target_labels=seg_target_labels),
        ConcatItemsd(keys=image_keys, name=concat_key, dim=0),
        DeleteItemsd(keys=image_keys),
        # Resized(keys=[concat_key], spatial_size=input_size, mode=resize_mode),
    ])
    ...
    if perform_data_aug:
        train_transforms = Compose([
            ...
            RandAffined(keys=[concat_key], prob=data_aug_p, scale_range=(0.07 * data_aug_strength, 0.07 * data_aug_strength, 0.07 * data_aug_strength),
                        padding_mode='border', mode=modes_2d),  # 3D: (num_channels, H, W[, D])
            ...
            RandRotated(keys=[concat_key], prob=data_aug_p, range_x=(np.pi / 24) * data_aug_strength, align_corners=align_corners_exception_2d,
            ...
            RandShiftIntensityd(keys=[concat_key], prob=data_aug_p, offsets=(0, 0.05 * data_aug_strength)),
...
def get_dataloaders(train_dict, val_dict, test_dict, train_transforms, val_transforms, test_transforms, 
    ...
    # val_dl_args_dict = {'dataset': val_ds, 'batch_size': 1, 'shuffle': False, 'num_workers': 0,
                        'drop_last': drop_last}
    test_dl_args_dict = {'dataset': test_ds, 'batch_size': 1, 'shuffle': False, 'num_workers': int(num_workers//2),
    # test_dl_args_dict = {'dataset': test_ds, 'batch_size': 1, 'shuffle': False, 'num_workers': 0,
                         'drop_last': drop_last}
...
def main(train_dict, val_dict, test_dict, features, perform_data_aug, data_aug_p, data_aug_strength, rand_cropping_size,
    ...
    # modes_3d = [config.ct_interpol_mode_3d, config.rtdose_interpol_mode_3d, config.segmentation_interpol_mode_3d]
    # modes_2d = [config.ct_interpol_mode_2d, config.rtdose_interpol_mode_2d, config.segmentation_interpol_mode_2d]
    ...
    train_transforms, val_transforms = get_transforms(perform_data_aug=perform_data_aug, modes_3d=modes_3d,
                                                      modes_2d=modes_2d, data_aug_p=data_aug_p, data_aug_strength=data_aug_strength,
